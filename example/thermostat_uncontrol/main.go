...
func init() {
	...
	flag.UintVar(&config.Port, "p", 10729, "homekit port, example: 10101, 10102...")
...
func main() {
	acc := homekit.NewAccessoryThermostat(config.GetInfo("Ex-Therm"), 3, 3, 3, 0)
	...
	go func() {
		...
		for range time.Tick(time.Millisecond * 5000) {
			if acc.Thermostat.TargetTemperature.Value() == acc.Thermostat.CurrentTemperature.Value() {
