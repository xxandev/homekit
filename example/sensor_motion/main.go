...
func init() {
	...
	flag.StringVar(&config.Name, "n", "SMotion", "homekit accessory name")
	flag.StringVar(&config.SN, "sn", "Ex-SMotion", "homekit accessory serial number")
	flag.StringVar(&config.Host, "h", "", "homekit host, example: 192.168.1.xxx")
	flag.UintVar(&config.Port, "p", 10721, "homekit port, example: 10101, 10102...")
...
func main() {
	acc := homekit.NewAccessorySensorMotion(config.GetInfo("Ex-SMotion"))
	...
	go func() {
		for range time.Tick(time.Millisecond * 3000000) {
			...
			log.Printf("update motion detected: %[1]T - %[1]v\n", acc.MotionSensor.MotionDetected.Value())
		}
	}()
