...
func main() {
	// log.Debug.Enable()
	acc := homekit.NewAccessoryThermostatAutomatic(accessory.Info{Name: "Thermostat", SerialNumber: "Ex-Therm-Non", Model: "HAP-TRM-NON", Manufacturer: homekit.Manufacturer, FirmwareRevision: homekit.Revision})
	...
	go acc.Switch.On.OnValueRemoteUpdate(func(v bool) {
		fmt.Printf("acc thermostat remote update on: %T - %v \n", v, v)
	})
