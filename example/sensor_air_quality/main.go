...
func init() {
	...
	flag.StringVar(&config.Name, "n", "AirQuality", "homekit accessory name")
	flag.StringVar(&config.SN, "sn", "Ex-AirQlt", "homekit accessory serial number")
	flag.StringVar(&config.Host, "h", "", "homekit host, example: 192.168.1.xxx")
	flag.UintVar(&config.Port, "p", 10714, "homekit port, example: 10101, 10102...")
...
func main() {
	acc := homekit.NewAccessorySensorAirQuality(config.GetInfo("Ex-AirQlt"))
	...
	go func() {
		for range time.Tick(time.Millisecond * 300000) {
			if acc.AirQualitySensor.AirQuality.Value() >= acc.AirQualitySensor.AirQuality.MaxValue() {
				acc.AirQualitySensor.AirQuality.SetValue(acc.AirQualitySensor.AirQuality.MinValue())
			} else {
				acc.AirQualitySensor.AirQuality.SetValue(acc.AirQualitySensor.AirQuality.Value() + 1)
			}
			fmt.Printf("update current state: %[1]T - %[1]v \n", acc.AirQualitySensor.AirQuality.Value())
		}
	}()
