...
func init() {
	...
	flag.StringVar(&config.Name, "n", "SLeak", "homekit accessory name")
	flag.StringVar(&config.SN, "sn", "Ex-SLeak", "homekit accessory serial number")
	flag.StringVar(&config.Host, "h", "", "homekit host, example: 192.168.1.xxx")
	flag.UintVar(&config.Port, "p", 10718, "homekit port, example: 10101, 10102...")
...
func main() {
	acc := homekit.NewAccessorySensorLeak(config.GetInfo("Ex-SLeak"))
