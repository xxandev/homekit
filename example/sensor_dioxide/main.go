...
func init() {
	...
	flag.StringVar(&config.Name, "n", "SDioxide", "homekit accessory name")
	flag.StringVar(&config.SN, "sn", "Ex-SDioxide", "homekit accessory serial number")
	flag.StringVar(&config.Host, "h", "", "homekit host, example: 192.168.1.xxx")
	flag.UintVar(&config.Port, "p", 10716, "homekit port, example: 10101, 10102...")
...
func main() {
	acc := homekit.NewAccessorySensorDioxide(config.GetInfo("Ex-SDioxide"))
	...
	go func() {
		for range time.Tick(time.Millisecond * 3000000) {
			acc.CarbonDioxideSensor.CarbonDioxideDetected.SetValue(int(math.Pow(0, float64(acc.CarbonDioxideSensor.CarbonDioxideDetected.Value()))))
			log.Printf("update on: %[1]T - %[1]v\n", acc.CarbonDioxideSensor.CarbonDioxideDetected.Value())
		}
	}()
