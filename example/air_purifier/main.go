...
func main() {
	...
	// log.Debug.Enable()
	acc := homekit.NewAccessoryAirPurifier(accessory.Info{Name: accessoryName, SerialNumber: accessorySn, Manufacturer: "alpr777", Model: "ACC-TEST", FirmwareRevision: "1.2"})
	...
	go acc.AirPurifier.TargetAirPurifierState.OnValueRemoteUpdate(func(v int) {
		...
		acc.AirPurifier.CurrentAirPurifierState.SetValue(v)
