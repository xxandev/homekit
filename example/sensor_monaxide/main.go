...
func init() {
	...
	flag.StringVar(&config.Name, "n", "SMonoxide", "homekit accessory name")
	flag.StringVar(&config.SN, "sn", "Ex-SMonoxide", "homekit accessory serial number")
	flag.StringVar(&config.Host, "h", "", "homekit host, example: 192.168.1.xxx")
	flag.UintVar(&config.Port, "p", 10720, "homekit port, example: 10101, 10102...")
...
func main() {
	acc := homekit.NewAccessorySensorMonoxide(config.GetInfo("Ex-SMonoxide"))
	...
	go func() {
		for range time.Tick(time.Millisecond * 3000000) {
			acc.CarbonMonoxideSensor.CarbonMonoxideDetected.SetValue(int(math.Pow(0, float64(acc.CarbonMonoxideSensor.CarbonMonoxideDetected.Value()))))
			fmt.Printf("update current state: %[1]T - %[1]v \n", acc.CarbonMonoxideSensor.CarbonMonoxideDetected.Value())
		}
	}()
