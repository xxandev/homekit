...
func init() {
	...
	flag.StringVar(&config.Name, "n", "SLight", "homekit accessory name")
	flag.StringVar(&config.SN, "sn", "Ex-SLight", "homekit accessory serial number")
	flag.StringVar(&config.Host, "h", "", "homekit host, example: 192.168.1.xxx")
	flag.UintVar(&config.Port, "p", 10719, "homekit port, example: 10101, 10102...")
...
func main() {
	acc := homekit.NewAccessorySensorLight(config.GetInfo("Ex-SLight"))
	...
	go func() {
		for range time.Tick(time.Millisecond * 1000) {
			...
			log.Printf("update current ambient light: %[1]T - %[1]v\n", acc.LightSensor.CurrentAmbientLightLevel.Value())
		}
	}()
