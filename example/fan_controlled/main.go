...
func main() {
	...
	// log.Debug.Enable()
	acc := homekit.NewAccessoryFanControlled(accessory.Info{Name: accessoryName, SerialNumber: accessorySn, Manufacturer: "EXAMPLE", Model: "ACC-TEST", FirmwareRevision: "1.2"})
	...
	go acc.Fan.RotationSpeed.OnValueRemoteUpdate(func(state float64) { fmt.Printf("acc remote update rotation speed: %T - %v \n", state, state) })
