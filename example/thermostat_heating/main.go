...
func init() {
	...
	flag.UintVar(&config.Port, "p", 10728, "homekit port, example: 10101, 10102...")
...
func main() {
	acc := homekit.NewAccessoryThermostat(config.GetInfo("Ex-Therm"), 0, 0, 1, 1)
	...
	go func() {
		for {
			...
			case <-time.Tick(time.Millisecond * 5000):
				if acc.Thermostat.CurrentHeatingCoolingState.Value() == 0 {
					if acc.Thermostat.TargetTemperature.Value()-10 > acc.Thermostat.CurrentTemperature.Value() {
