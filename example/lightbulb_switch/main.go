...
const (
	...
	SN      string = "EX-Switch"
	MODEL   string = "HAP-LBSW"
	ADDRESS string = ":11111"
	PIN     string = "12344321"
)

func main() {
	homekit.OnLog(false)
	acc := homekit.NewAccessoryLightbulbSwitch(accessory.Info{Name: NAME, SerialNumber: SN, Model: MODEL, Manufacturer: homekit.Manufacturer, Firmware: homekit.Firmware})
	...
	go func() {
		for range time.Tick(30 * time.Second) {
			...
			llog.Printf("acc lightbulb update on: %T - %v \n", acc.LightbulbSwitch.On.Value(), acc.LightbulbSwitch.On.Value())
		}
	}()
