...
func init() {
	...
	flag.StringVar(&config.Name, "n", "Window", "homekit accessory name")
	flag.StringVar(&config.SN, "sn", "Ex-Window", "homekit accessory serial number")
	flag.StringVar(&config.Host, "h", "", "homekit host, example: 192.168.1.xxx")
	flag.UintVar(&config.Port, "p", 10731, "homekit port, example: 10101, 10102...")
...
func main() {
	acc := homekit.NewAccessoryWindow(config.GetInfo("Ex-Window"))
