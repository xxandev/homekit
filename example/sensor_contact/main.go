...
func init() {
	...
	flag.StringVar(&config.Name, "n", "SContact", "homekit accessory name")
	flag.StringVar(&config.SN, "sn", "Ex-SContact", "homekit accessory serial number")
	flag.StringVar(&config.Host, "h", "", "homekit host, example: 192.168.1.xxx")
	flag.UintVar(&config.Port, "p", 10715, "homekit port, example: 10101, 10102...")
...
func main() {
	acc := homekit.NewAccessorySensorContact(config.GetInfo("Ex-SContact"))
	...
	go func() {
		for range time.Tick(time.Millisecond * 300000) {
			...
			log.Printf("update on: %[1]T - %[1]v\n", acc.ContactSensor.ContactSensorState.Value())
		}
	}()
