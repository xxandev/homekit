...
func (s *stream) start(video rtp.VideoParameters, audio rtp.AudioParameters) error {
	...
	if s.inputDevice == "rtsp" {
		...
		// audio support may be added later		//                fmt.Sprintf(" -map 0:1 -acodec libfdk_aac -profile:a aac_eld -flags +global_header -f null -ar 16k -b:a 24k -bufsize 24k -ac 1 -payload_type 110")+
